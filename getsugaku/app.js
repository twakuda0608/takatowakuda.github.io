...
function occurrenceInNextMonth(nextDateStr, freqMonths) {
  const now   = new Date();
  const start = new Date(now.getFullYear(), now.getMonth() + 1, 1);
  const end   = new Date(now.getFullYear(), now.getMonth() + 2, 0);
...
function render() {
  ...
  payments.forEach(p => {
    ...
    const nextMonthEff = occurrenceInNextMonth(p.nextPaymentDate, freq);
  ...
  CAT_ORDER.forEach(cat => {
    ...
    html += items.map(p => {
      ...
      const dueNext = occurrenceInNextMonth(p.nextPaymentDate, freq) !== null;
